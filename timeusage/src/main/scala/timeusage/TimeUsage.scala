...
object TimeUsage extends TimeUsageInterface:
    ...
    def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
        val primaryNeeds: List[Column] = columnNames.filter(
            colName => List("t01", "t03", "t11", "t1801", "t1803")
                .exists(prefix => colName.startsWith(prefix))).map(col)
        val workingActivities: List[Column] = columnNames.filter(
            colName => List("t05", "t1805")
                .exists(prefix => colName.startsWith(prefix))).map(col)
        val otherActivities: List[Column] = columnNames.filter(x => !primaryNeeds.contains(col(x)) && !workingActivities.contains(col(x)))
            .filter(colName => List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
                .exists(prefix => colName.startsWith(prefix))).map(col)
    ...
    def timeUsageSummary(primaryNeedsColumns: List[Column],
        ...
        val sexProjection: Column = when(col("tesex").eqNullSafe(1), "male").otherwise("female").as("sex")
        ...
        val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _).cast(DoubleType) / 60).as("primaryNeeds")
        val workProjection: Column = (workColumns.reduce(_ + _).cast(DoubleType) / 60).as("work")
        val otherProjection: Column = (otherColumns.reduce(_ + _).cast(DoubleType) / 60).as("other")
        df
